 ...
 */
export async function ensureIncomeForDevis(prisma: PrismaClient, devisId: string) {
  const devis = await prisma.crmRecord.findUnique({ where: { id: devisId } });
  if (!devis || devis.kind !== "devis") return null;
  const d = (devis.data ?? {}) as Record<string, any>;
  ...
  if (d.event_id) {
    ...
    if (byEvent) {
      const merged = {
        ...((byEvent.data ?? {}) as Record<string, any>),
        devis_id: devisId,
        client_id: d.client_id ?? (byEvent.data as any)?.client_id ?? null,
        event_id: d.event_id,
        reference_facture: incomeData.reference_facture,
        description: incomeData.description,
        montant: incomeData.montant || ((byEvent.data as any)?.montant ?? 0),
      };
 ...
 */
export async function ensureIncomeForEvent(
  prisma: PrismaClient,
  event: { id: string; title: string; price?: number | null; clientId?: string | null },
) {
  const existing = await prisma.crmRecord.findFirst({
