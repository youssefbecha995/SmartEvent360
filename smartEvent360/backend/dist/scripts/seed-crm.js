...
async function main() {
    ...
    const count = (kind) => prisma.crmRecord.count({ where: { kind } });
    const clear = (kind) => prisma.crmRecord.deleteMany({ where: { kind } });
    // ── Client de démo ─────────────────────────────────────────────
    ...
    if ((await count("services")) === 0) {
        ...
        for (const s of services)
            await prisma.crmRecord.create({ data: { kind: "services", data: s } });
