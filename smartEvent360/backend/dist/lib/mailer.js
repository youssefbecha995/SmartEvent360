...
const mailFrom = () => process.env.MAIL_FROM || (process.env.SMTP_USER ? `SmartEvent360 <${process.env.SMTP_USER}>` : "SmartEvent360 <noreply@smartevent360.com>");
const fmtDateTime = (d) => d ? new Date(d).toLocaleString("fr-FR", { dateStyle: "full", timeStyle: "short" }) : "–";
async function sendAppointmentConfirmation(opts) {
    const tr = getTransporter();
    if (!tr)
        return { sent: false, reason: "SMTP non configuré (SMTP_USER/SMTP_PASS manquants)" };
