...
function isOwner(row, userId) {
    return row.data?.client_id === userId;
}
// GET /api/client/devis — devis du client connecté
router.get("/devis", async (req, res) => {
    const rows = await prisma_1.prisma.crmRecord.findMany({ where: { kind: "devis" }, orderBy: { createdAt: "asc" } });
    res.json(rows.filter(r => isOwner(r, req.user.userId)).map(toRecord));
});
// GET /api/client/devis/:id
router.get("/devis/:id", async (req, res) => {
    const row = await prisma_1.prisma.crmRecord.findUnique({ where: { id: req.params.id } });
...
router.post("/devis/:id/accept", async (req, res) => {
    const row = await prisma_1.prisma.crmRecord.findUnique({ where: { id: req.params.id } });
    if (!row || row.kind !== "devis" || !isOwner(row, req.user.userId)) {
        res.status(404).json({ error: "Devis not found" });
        return;
    ...
    const income = await (0, treasury_1.ensureIncomeForDevis)(prisma_1.prisma, req.params.id);
    // Notifier les admins : devis accepté
    const clientName = await (0, notify_1.userDisplayName)(req.user.userId);
    (0, notify_1.notifyAdmins)({
        type: "SUCCESS",
        title: "Devis accepté",
        message: `${clientName} a accepté le devis ${row.data?.numero || ""}`.trim(),
        lien: "/admin/devis",
    }).catch((e) => console.error("[notify]", e));
    res.json({ id: row.id, ...merged, income: income ? toRecord(income) : null });
});
// POST /api/client/devis/:id/refuse
router.post("/devis/:id/refuse", async (req, res) => {
    const row = await prisma_1.prisma.crmRecord.findUnique({ where: { id: req.params.id } });
    if (!row || row.kind !== "devis" || !isOwner(row, req.user.userId)) {
        res.status(404).json({ error: "Devis not found" });
        return;
    ...
    (0, notify_1.notifyAdmins)({
        type: "WARNING",
        title: "Devis refusé",
        message: `${clientName} a refusé le devis ${row.data?.numero || ""}${reason ? ` — Raison : ${reason}` : ""}`.trim(),
        lien: "/admin/devis",
    }).catch((e) => console.error("[notify]", e));
...
router.get("/payments", async (req, res) => {
    const rows = await prisma_1.prisma.crmRecord.findMany({ where: { kind: "incomes" }, orderBy: { createdAt: "asc" } });
    res.json(rows.filter(r => isOwner(r, req.user.userId)).map(toRecord));
});
...
router.post("/payments/:id/pay", async (req, res) => {
    ...
    const merged = {
        ...row.data,
        statut: "paye",
        date_paiement: (0, treasury_1.today)(),
        mode_paiement: req.body?.methode || "en_ligne",
        numero_reçu: req.body?.methode === "en_ligne" ? `REC-${Date.now().toString().slice(-8)}` : row.data?.numero_reçu,
    };
    await prisma_1.prisma.crmRecord.update({ where: { id: req.params.id }, data: { data: merged } });
    res.json(toRecord({ id: row.id, data: merged }));
});
function listForOwner(kind, userId) {
    return prisma_1.prisma.crmRecord
        .findMany({ where: { kind }, orderBy: { createdAt: "asc" } })
        .then(rows => rows.filter(r => isOwner(r, userId)).map(toRecord));
}
function findOwned(kind, id, userId) {
    return prisma_1.prisma.crmRecord.findUnique({ where: { id } }).then(row => {
        if (!row || row.kind !== kind || !isOwner(row, userId))
            return null;
        return row;
    });
}
...
router.post("/appointments", async (req, res) => {
    const body = (req.body ?? {});
    const row = await prisma_1.prisma.crmRecord.create({
        data: {
            kind: "appointments",
            data: {
    ...
    const clientName = await (0, notify_1.userDisplayName)(req.user.userId);
    const when = (0, notify_1.displayDate)(body.date_heure);
    (0, notify_1.notifyAdmins)({
        type: "INFO",
...
router.patch("/appointments/:id", async (req, res) => {
    ...
    if (statut === "annule" || statut === "reporte") {
        ...
        (0, notify_1.notifyAdmins)({
            ...
            message: `${clientName} a ${statut === "annule" ? "annulé" : "reporté"} son rendez-vous du ${(0, notify_1.displayDate)(row.data?.date_heure) || "(date inconnue)"}.`,
            lien: "/admin/rendez-vous",
        }).catch((e) => console.error("[notify]", e));
    }
    res.json(toRecord({ id: row.id, data: merged }));
...
router.get("/packs", async (req, res) => {
    ...
    const mine = rows
        .filter(r => r.data?.client_id === req.user.userId)
        .map(toRecord);
    const packIds = [...new Set(mine.map(r => r.pack_id).filter(Boolean))];
    const packs = packIds.length
        ? await prisma_1.prisma.pack.findMany({ where: { id: { in: packIds } } })
        : [];
    const packMap = new Map(packs.map(p => [p.id, p]));
    res.json(mine.map(r => ({ ...r, pack: r.pack_id ? packMap.get(r.pack_id) ?? null : null })));
});
// POST /api/client/packs/check-availability — vérifie la disponibilité d'un pack à une date
router.post("/packs/check-availability", async (req, res) => {
    const { packId, date } = req.body ?? {};
    if (!packId || !date) {
        res.status(400).json({ error: "packId et date sont requis." });
        return;
    }
    const pack = await prisma_1.prisma.pack.findUnique({ where: { id: String(packId) } });
    ...
    const rows = await prisma_1.prisma.crmRecord.findMany({ where: { kind: "client_packs" } });
...
router.post("/packs", async (req, res) => {
    ...
    const row = await prisma_1.prisma.crmRecord.create({ data: { kind: "client_packs", data } });
...
router.patch("/packs/:id", async (req, res) => {
    ...
    const statut = req.body?.statut ?? row.data?.statut;
    const merged = {
        ...row.data,
        statut,
        notes_annulation: req.body?.notes ?? row.data?.notes_annulation ?? null,
        date_annulation: statut === "annule" ? new Date().toISOString() : row.data?.date_annulation ?? null,
    };
    await prisma_1.prisma.crmRecord.update({ where: { id: req.params.id }, data: { data: merged } });
    if (statut === "annule") {
        const clientName = await (0, notify_1.userDisplayName)(req.user.userId);
        (0, notify_1.notifyAdmins)({
            ...
            message: `${clientName} a annulé sa réservation du pack « ${row.data?.nom_pack || ""} » (${row.data?.date_debut || ""}).`,
            lien: "/admin/reservations",
        }).catch((e) => console.error("[notify]", e));
    }
    res.json(toRecord({ id: row.id, data: merged }));
...
router.post("/quotes", async (req, res) => {
    const body = (req.body ?? {});
    const count = await prisma_1.prisma.crmRecord.count({ where: { kind: "devis" } });
    const reference = `DEV-${new Date().getFullYear()}-${String(count + 1).padStart(3, "0")}`;
    let pack = null;
    if (body.pack_id) {
        const found = await prisma_1.prisma.pack.findUnique({ where: { id: String(body.pack_id) } });
        if (found)
            pack = found;
    }
    const data = {
    ...
    const row = await prisma_1.prisma.crmRecord.create({ data: { kind: "devis", data } });
    const clientName = await (0, notify_1.userDisplayName)(req.user.userId);
    (0, notify_1.notifyAdmins)({
        type: "INFO",
