...
import { packsApi, clientApi, NeonPack, NeonPackService } from '@/lib/neonApi';
                ...
                >
                  ...
                  <div className="flex flex-col flex-1 p-6 pt-5">
                    ...
                    <div className="mb-4">
                      ...
                      {pack.originalPrice && pack.originalPrice > pack.prix && (
                        <p className="text-green-400 text-xs font-medium">
                          Économisez {(pack.originalPrice - pack.prix).toLocaleString('fr-FR')} DT
                          <span className="text-dark-500 line-through ml-2">{pack.originalPrice.toLocaleString('fr-FR')} DT</span>
                        </p>
                      )}
                    ...
                    {(pack.packServices && pack.packServices.length > 0) ? (
                      <div className="mb-5">
                        ...
                        <ul className="space-y-1.5">
                          {pack.packServices.slice(0, MAX_FEATURES_IN_CARD).map((ps) => (
                            <li key={ps.id} className="flex items-center gap-2 text-xs text-dark-200">
                          ...
                          {pack.packServices.length > MAX_FEATURES_IN_CARD && (
                            <p className="text-xs text-gold-500/90 mt-2 font-medium">+ {pack.packServices.length - MAX_FEATURES_IN_CARD} {t('prestations supplémentaires')}</p>
                          )}
                        </ul>
                      </div>
                    ) : visible.length > 0 ? (
              ...
              {step === 'detail' && (
                <div>
                  ...
                  <div className="p-5">
                    ...
                    {((modalPack.packServices && modalPack.packServices.length > 0) || (modalPack.features || []).length > 0) && (
                      <div className="bg-dark-700/40 rounded-xl p-4 mb-4">
                        ...
                        {modalPack.packServices && modalPack.packServices.length > 0 ? (
                          <ul className="space-y-1.5">
                            {modalPack.packServices.map((ps) => (
                              <li key={ps.id} className="flex items-center gap-2 text-xs text-dark-200">
                                <span className="w-4 h-4 rounded-full bg-green-500/20 text-green-400 border border-green-500/40 flex items-center justify-center flex-shrink-0">
                                  <Check size={9} strokeWidth={3} />
                                </span>
